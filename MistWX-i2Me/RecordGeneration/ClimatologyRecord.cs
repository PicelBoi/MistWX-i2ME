...
public class ClimatologyRecord : I2Record
{
    public async Task<string> MakeRecord(List<GenericResponse<Almanac1DayResponse>> results)
    {
        ...
        foreach (var result in results)
        {
            ...
            cliRec.AvgHigh = result.ParsedData.temperatureAverageMax;
            cliRec.AvgLow = result.ParsedData.temperatureAverageMin;
            cliRec.RecHigh = result.ParsedData.temperatureRecordMax;
            cliRec.RecLow = result.ParsedData.temperatureRecordMin;
            cliRec.RecHighYear = result.ParsedData.almanacRecordYearMax;
            cliRec.RecLowYear = result.ParsedData.almanacRecordYearMin;
